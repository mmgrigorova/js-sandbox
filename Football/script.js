...
// Test data for 6.: First, use players 'Davies', 'Muller', 'Lewandowski' and 'Kimmich'.Then, call the function again with players from game.

// usage of: destructuring arrays, objects, the spread operator, the rest pattern, short circuiting || &&, nullish coalesce ??, logical assignment
...
const game = {
    ...
    printGoals(...playerNames) {
        ...
        for (let i = 0; i < playerNames.length; i++) {
            playersString += playerNames[i];
            playersString += ', '
        }
        console.log(`The goals were made by ${playersString}${playerNames.length} in total`);
    }
};
...
// 7.
console.log(`The team is more likely to win is 
    ${(game.odds.team1 < game.odds.team2) && ('team 1') || ('team 2')}`);
